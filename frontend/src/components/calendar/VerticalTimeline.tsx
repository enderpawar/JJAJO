...
export function VerticalTimeline({ skipNextScrollToTimeRef }: VerticalTimelineProps = {}) {
  ...
  // 단, 버튼(편집·완료·삭제 등) 터치 시에는 preventDefault 하지 않음 → 클릭 이벤트 정상 발생
  useEffect(() => {
    const el = timelineRef.current
    if (!el) return
    const handleTouchStart = (e: TouchEvent) => {
      const target = e.target as Element
      const onCard = target.closest?.('.task-card')
      const isEditing = target.closest?.('[data-editing-card]')
      const isButton = target.closest?.('button, [role="button"], input')
      if (onCard && !isEditing && !isButton) {
        e.preventDefault()
      }
    }
  ...
  return (
    <>
    <div ref={timelineRef} className={`timeline-scroll flex-1 min-h-0 theme-transition bg-theme relative pl-5 pr-2 sm:px-4 ${isCardDragging ? 'timeline-scroll-dragging' : ''}`} data-tour="timeline">
